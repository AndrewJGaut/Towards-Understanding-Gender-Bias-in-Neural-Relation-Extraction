...
class Embedding(object):
    ...
    #high level: word is word that will complete analogy, which we get using
    #the classic analogy vector subtraction with 3 other words
    #not sure how you'd adjust this to be able to create analogies using just 
    #2 seed words
    def nearest_neighbors(self, word, k=1, exclude=[], metric="cosine"):
        ...
        #below, return a numpy matrix of cosine similarities between arrays in vectors and v.reshape(1,-1)
        #NOTE: reshape takes an array and changes its dimensions. -1 signifies that numpy should automatically
        #create that dimension. v.reshape changes v into a one-row vector.
        #metric is cosine_similarity
        D = pairwise_distances(self.vectors, v.reshape(1, -1), metric=metric) 

        if isinstance(word, string_types):
            D[self.vocabulary.word_id[word]] = D.max()

        #set entries for each word in analogy task to be maximum value of similarity found in distance matrix
        #so that these words are omitted from answer
        for w in exclude:
            D[self.vocabulary.word_id[w]] = D.max()

        #argsort: return the indices that would create a sorted array
        #axis in argsort: specifies that D will be argsorted by COLUMN (i.e. it'll be argsorted downwards)
        #rather than by row (i.e. argsorted across) ([6, 5], [4, 1] --> [1, 1], [0,0])
        #flatten transforms the output into a single array ([1,2], [3,4] --> [1,2,3,4])
        #0:k gives the first k outputs
