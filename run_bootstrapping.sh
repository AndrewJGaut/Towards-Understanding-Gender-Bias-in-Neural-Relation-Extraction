...
do
	...
	python3 -u WikigenderJsonParsing/DebiasJsonDataset.py ${deboptions[($i)]} -bs & 
	wait
	...
	do
		...
		python3 -u WikigenderJsonParsing/CreateBootstrappedDatasets.py ${deboptions[($i)]} -bs & 
		wait
		echo "done creating bootstrapped dataset"
		python3 -u WikigenderJsonParsing/ConvertWikigenderToOpenNREFormat.py ${deboptions[($i)]} -bs & 
		wait
		echo "done convertin to opennre format"

		python3 -u WordEmbeddings/Word2VecTraining.py ${deboptions[($i)]}  -bs &
		wait
		echo "done creating and debiasing embeddings"
		python3 -u Models/OpenNRE/train_debiasingoptions.py ${deboptions[($i)]} -bs &
		wait
		echo "done training model" 
		python3 -u Models/OpenNRE/test_debiasingoptions.py ${deboptions[($i)]} -bs -bs_num=$j &
		wait
		echo "done testing model on female data" 
		python3 -u Models/OpenNRE/test_debiasingoptions.py ${deboptions[($i)]} --male_test_files -bs -bs_num=$j &
		wait
