...
do
	...
	do
		...
		CUDA_VISIBLE_DEVICES=$gpu python3 -u Models/OpenNRE/train_debiasingoptions.py ${deboptions[($i)]} -bs --data_parallel  &
		wait
		echo "done training model" 
		CUDA_VISIBLE_DEVICES=$gpu python3 -u Models/OpenNRE/test_debiasingoptions.py ${deboptions[($i)]} -bs -bs_num=$j --data_parallel &
		wait
		echo "done testing model on female data" 
		CUDA_VISIBLE_DEVICES=$gpu python3 -u Models/OpenNRE/test_debiasingoptions.py ${deboptions[($i)]} --male_test_files -bs -bs_num=$j --data_parallel &
		wait
