...
class json_file_data_loader(file_data_loader):
    ...
    def __init__(self, file_name, word_vec_file_name, rel2id_file_name, mode, shuffle=True, max_length=120, case_sensitive=False, reprocess=False, batch_size=160, bootstrapping = False):
        '''
        ...
        if reprocess or not self._load_preprocessed_file(): # Try to load pre-processed files:
            ...
            if not os.path.exists(file_name):
                raise Exception("[ERROR] Data file {} doesn't exist".format(file_name))
            ...
            # for bootstrapping, we take only a subset of the data
            if bootstrapping:
                self.ori_data = random.sample(self.ori_data, 0.8 * len(self.ori_data))
